...
int main(int argc, char *argv[]) {
    ...
    for (int i=0; i<nTimes; ++i) {
      ...
      if (nTimes >= 10 && (i+1)%(nTimes/10) == 0) {
        printf("process:%d/%d, time:%f estimate remain:%fs\n", i+1, nTimes, (double(clock()-timeStart)/CLOCKS_PER_SEC), double(nTimes-i-1)/double(i+1)*(double(clock()-timeStart)/CLOCKS_PER_SEC));
